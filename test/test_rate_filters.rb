...
#Standard working test of all fields
  ...
  it 'prints a valid configuration line' do
    post "/thresholds/rate_filter/new?sid=123&gid=456&track_by=src&count=10&new_action=drop&seconds=60&timeout=60&comment=#{CGI.escape 'Finding the pit of success [smm]'}&api_key=#{MASTERKEY}"
    json = JSON.parse(last_response.body)
  ...
  it 'should raise an Invalid RateFilter Object Error' do
    post "/thresholds/rate_filter/new?sid=123&track_by=src&count=10&new_action=drop&seconds=60&timeout=60&api_key=#{MASTERKEY}"
  ...
  it 'should create and delete rate_filters' do
    ...
    post "/thresholds/rate_filter/delete?sid=123&gid=456&api_key=#{MASTERKEY}"
    expect(last_response.ok?).to eq true
  end
