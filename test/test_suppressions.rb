...
describe SnortThresholdsRest::Server do
  ...
  it 'prints a valid configuration line' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=src&ip=1.2.3.4'
    json = JSON.parse(last_response.body)
  ...
  it 'prints a valid configuration line' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=dst&ip=1.2.3.4'
    json = JSON.parse(last_response.body)
  ...
  it 'prints a valid configuration line' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=dst&ip=100.200.103.114'
    json = JSON.parse(last_response.body)
  ...
  it 'prints a valid configuration line' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=dst&ip=1.2.3.4/20'
    json = JSON.parse(last_response.body)
  ...
  it 'prints a valid configuration line' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=dst&ip=1.2.3.4/8'
    json = JSON.parse(last_response.body)
  ...
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=src&ip=1.2.3.4/208'
    expect(last_response.ok?).to eq false
  end

  #Test failure of no IP set but Track by_ set
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=src'
    expect(last_response.ok?).to eq false
  end

  #Test failure on SID and Track by_ set with no GID or IP set
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?sid=123&track_by=src'
    expect(last_response.ok?).to eq false
  end

  #Test failure on SID set but no GID
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?sid=123'
    expect(last_response.ok?).to eq false
  end

  #Test failure on GID set with no SID
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?gid=234'
    expect(last_response.ok?).to eq false
  end

  #Test failure on too many octects in IP
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=src&ip=1.2.3.4.5'
    expect(last_response.ok?).to eq false
  end

  #Test failure on invalid IP address
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=src&ip=100.200.300.400'
    expect(last_response.ok?).to eq false
  end

  #Test failure on invalid CIDR range of 0
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?sid=123&gid=456&track_by=src&ip=1.2.3.4/0'
    expect(last_response.ok?).to eq false
  end

  #Test failure if SID contains letters
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?sid=123a&gid=456&track_by=src&ip=1.2.3.4'
    expect(last_response.ok?).to eq false
  end

  #Test failure if GID contains letters
  it 'should raise an Invalid Suppression Object Error' do
    post '/thresholds/suppression/new?sid=123&gid=456a&track_by=src&ip=1.2.3.4'
    expect(last_response.ok?).to eq false
  end
